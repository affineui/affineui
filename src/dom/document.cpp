...
#if !defined(AFFINEUI_STUB_BUILD)
// Forward declarations of anon-namespace helpers defined later in this TU.
...
std::string select_display_text(lxb_dom_element_t* select) {
    ...
    for (auto* c = lxb_dom_node_first_child(lxb_dom_interface_node(select));
        ...
        if (detail::has_attr(el, "selected")) return node_text(c);
    }
...
std::string generated_content_text(std::string value,
                                   const detail::ResolvedStyle& style) {
    value = std::string(detail::trim_css_ws(substitute_style_vars(value, style)));
...
void append_generated_content_for_element(
    ...
    if (!display_value.empty() &&
        detail::trim_css_ws(substitute_style_vars(display_value, elem_style)) ==
            "none") {
        return;
    }
...
void collect_blocks(detail::DocumentImpl& impl,
    ...
    for (auto* child = lxb_dom_node_first_child(node); child;
        ...
        if (!is_block_tag(tag)) {
            ...
            const auto cls_attr = split_classes(detail::attr_string(elem, "class"));
...
void Document::set_html(std::string_view html) {
    ...
    if (body) {
        ...
        apply_font_family_fills(*impl_, "body", detail::attr_string(body_elem, "id"),
                                split_classes(detail::attr_string(body_elem, "class")),
                                /*parent_idx=*/-1,
                                /*state_bits=*/0,
                                impl_->root_style);
    }
...
// alongside the dispatch helpers. (Cross-file helpers formerly declared
// here now live in internal/document_impl.h under namespace detail.)
namespace {
#if !defined(AFFINEUI_STUB_BUILD)
...
bool element_could_match_compound_modulo_attribute(
    ...
    const auto classes = split_classes(detail::attr_string(elem, "class"));
    for (const auto& s : compound.simples) {
...
void recollect_blocks_from_current_dom(detail::DocumentImpl& impl) {
    ...
    if (body && impl.resolver) {
        ...
        apply_font_family_fills(impl, "body", detail::attr_string(body_elem, "id"),
                                split_classes(detail::attr_string(body_elem, "class")),
                                /*parent_idx=*/-1,
                                /*state_bits=*/0,
                                impl.root_style);
...
void refresh_block_metadata_from_element(Block& block,
    ...
    block.classes = split_classes(detail::attr_string(elem, "class"));
...
lxb_dom_element_t* nearest_ancestor_with_class(lxb_dom_element_t* elem,
    ...
    for (auto* current = elem; current != nullptr;
        ...
        const auto classes = split_classes(detail::attr_string(current, "class"));
        if (std::find(classes.begin(), classes.end(), cls) != classes.end()) {
            return current;
        }
    }
...
lxb_dom_element_t* nearest_checkbox_wrapper(lxb_dom_element_t* elem) {
    ...
    for (auto* node = elem ? lxb_dom_interface_node(elem) : nullptr;
        ...
        const auto classes = split_classes(detail::attr_string(candidate, "class"));
...
bool pseudo_state_reveals_hidden_subtree(detail::DocumentImpl& impl,
    ...
    for (int idx = root_idx; idx < static_cast<int>(impl.blocks.size());
        ...
        for (auto* c = lxb_dom_node_first_child(lxb_dom_interface_node(elem));
            ...
            pseudo_block.classes = split_classes(detail::attr_string(child, "class"));
...
std::string decius_slider_thumb_style(double min, double max, double value) {
    return "left:" + percent_string(detail::normalized_control_value(value, min, max));
}
...
std::string class_list_set(lxb_dom_element_t* elem,
                           std::string_view cls,
                           bool present) {
    auto classes = split_classes(detail::attr_string(elem, "class"));
...
bool activate_dcs_menu_item(detail::DocumentImpl& impl,
    ...
    if (detail::has_attr(menu, "data-aui-colorfield") &&
        ...
        auto* colorfield =
            find_dom_element_by_id(impl, detail::attr_string(menu, "data-aui-colorfield"));
        bool changed = false;
        if (colorfield) {
            changed =
...
// dock_structure_changed() recollects the box tree once and the Document's
...
lxb_dom_element_t* dock_create_el(detail::DocumentImpl& impl,
    ...
    auto* e = lxb_dom_document_create_element(
        lxb_dom_interface_document(impl.doc), as_lxb(tag), tag.size(), nullptr);
    if (e && !classes.empty()) {
        lxb_dom_element_set_attribute(e, as_lxb("class"), 5, as_lxb(classes),
                                      classes.size());
    }
    return e;
}
...
void dock_set_attr(lxb_dom_element_t* e, std::string_view name,
                   std::string_view value) {
    if (!e) return;
    lxb_dom_element_set_attribute(e, as_lxb(name), name.size(), as_lxb(value),
                                  value.size());
}
...
std::string dock_kind_of(lxb_dom_element_t* pane);  // defined with the
                                                    // drop-zone geometry below

bool is_floating_dock(lxb_dom_element_t* dock) {
    return dock && dock_kind_of(dock) == "panels" &&
           ancestor_with_class(dock, "dcs-panel--floating") != nullptr;
}
...
void sync_dock_tab_shape(detail::DocumentImpl& impl, lxb_dom_element_t* dock) {
    (void) impl;
    if (!dock) return;
    const std::size_t count = dock_tabs(dock).size();
...
void activate_tab_in_dock(lxb_dom_element_t* dock, lxb_dom_element_t* tab) {
    ...
    for (auto* c = lxb_dom_node_first_child(lxb_dom_interface_node(body)); c;
        ...
        if (detail::attr_string(e, "id") == target_id) {
            lxb_dom_element_remove_attribute(e, as_lxb("hidden"), 6);
        } else {
    ...
    for (auto* tb : toolbars) {
        if (detail::attr_string(tb, "data-dcs-tabtoolbar") == sel) {
            lxb_dom_element_remove_attribute(tb, as_lxb("hidden"), 6);
        } else {
...
void prepare_tab_for_tabbar(detail::DocumentImpl& impl,
    ...
    lxb_dom_element_remove_attribute(tab, as_lxb("data-dcs-title-tab"), 18);
}
...
void ensure_tabbed_dock(detail::DocumentImpl& impl, lxb_dom_element_t* dock) {
    ...
    if (!title_tab) {
        if (tabbar) lxb_dom_element_remove_attribute(tabbar, as_lxb("hidden"), 6);
        sync_dock_tab_shape(impl, dock);
        return;
    }
...
bool dock_move_tab_to(detail::DocumentImpl& impl, lxb_dom_element_t* tab,
    ...
    auto* source = ancestor_with_class(tab, "dcs-dockpane");
...
void dock_unsplit_from_layout(detail::DocumentImpl& impl,
    ...
    auto* prev = previous_element_sibling(lxb_dom_interface_node(node));
    auto* next = next_element_sibling(lxb_dom_interface_node(node));
...
void dock_cleanup_source(detail::DocumentImpl& impl, lxb_dom_element_t* dock) {
    if (!dock) return;
    auto remaining = dock_tabs(dock);
    if (remaining.empty()) {
        if (auto* floater = ancestor_with_class(dock, "dcs-panel--floating")) {
...
void dock_split(detail::DocumentImpl& impl, lxb_dom_element_t* target,
    ...
    const int target_idx = block_index_for_exact_element(impl, target);
    ...
    if (parent_is_dock && parent_vertical == need_vertical) {
        ...
        std::string target_flex = find_decl_value(detail::attr_string(target, "style"),
                                                  "flex");
...
Document::DockLayout::Node dock_layout_node(lxb_dom_element_t* e) {
    ...
    n.flex = find_decl_value(detail::attr_string(e, "style"), "flex");
    ...
    // Leaf: a dockpane.
    n.kind = dock_kind_of(e);
    for (auto* tab : dock_tabs(e)) {
        const std::string id = dockpane_tab_panel_id(tab);
        if (id.empty()) continue;
        n.tabs.push_back(id);
...
void capture_tab_drag_metadata(detail::DocumentImpl& impl,
    ...
    if (tab) {
        drag.label = std::string(
            detail::trim_css_ws(node_text(lxb_dom_interface_node(tab))));
    }
...
int dcs_tree_row_depth(lxb_dom_element_t* row) {
    const std::string depth_value =
        find_decl_value(detail::attr_string(row, "style"), "--depth");
...
Document::DockLayout Document::dock_layout() const {
    ...
    for (auto* fp : floats) {
        ...
        auto px = [&](std::string_view prop) {
            const std::string v = find_decl_value(style, prop);
            return v.empty()
                       ? 0
...
Rect Document::find_element_rect(std::string_view target) const {
    ...
    if (!found) return {};
    const int bi = block_index_for_exact_element(*impl_, found);
    if (bi < 0) return {};
        ...
        case EventType::MouseMove: {
            ...
            if (impl_->tab_drag.tab) {
                ...
                if (impl_->tab_drag.dragging) {
                    ...
                    auto* drag_src_pane =
                        ancestor_with_class(drag_src_tab, "dcs-dockpane");
        ...
        case EventType::MouseUp: {
            ...
            if (impl_->tab_drag.tab) {
                ...
                if (td.dragging) {
                    ...
                    auto* tab = find_dockpane_tab_for_panel_id(*impl_,
                                                               td.panel_id);
                    auto* panel = find_dom_element_by_id(
                    ...
                    if (tab && panel && source) {
                        ...
                        if (t.self_noop) {
                            ...
                            if (changed_dock) {
                                dock_cleanup_source(*impl_, source);
                                dock_trace("dock panel=" + td.panel_id +
                                           " target=" + t.parent);
                            }
                        } else if (t.valid && t.pane) {
                            // Edge: a fresh pane owns the tab; split the target.
                            auto* fresh = dock_create_pane(*impl_, td.panel_id,
                                                           td.drag_kind);
                            if (fresh &&
                                dock_move_tab_to(*impl_, tab, panel, fresh)) {
                                dock_split(*impl_, t.pane, t.zone, fresh,
                                           t.window_edge);
                                dock_cleanup_source(*impl_, source);
                                changed_dock = true;
                                dock_trace(
                                    "edge-dock panel=" + td.panel_id +
                                    " edge=" + drop_zone_name(t.zone) +
                            ...
                            const int si =
                                block_index_for_exact_element(*impl_, source);
                            const Rect sb =
                            ...
                            w = positive_int_attr(tab, "data-dcs-tearout-width",
                                                  w);
                            h = positive_int_attr(tab,
                                                  "data-dcs-tearout-height", h);
                            auto* fp = dock_spawn_floating_panel(
                                *impl_, tab, panel, ev.pos, w, h, td.drag_kind,
                                td.panel_id);
                            if (fp) {
                                dock_cleanup_source(*impl_, source);
                                changed_dock = true;
                                dock_trace("tearoff panel=" + td.panel_id);
                            }
                        }
                    }
                    if (changed_dock) {
                        dock_structure_changed(*impl_);
                        dock_trace_state(*impl_, "after-dock-surgery");
