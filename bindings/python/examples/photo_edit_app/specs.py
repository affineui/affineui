...
# Display blend name → canvas composite-op value (what the raster core
# and the web's globalCompositeOperation use).
...
# Canvas value → display name (first writer wins: "color-dodge" reads
# back as "Color Dodge", not "Dodge").
CANVAS_TO_BLEND: dict[str, str] = {}
for _name, _value in BLEND_TO_CANVAS.items():
    CANVAS_TO_BLEND.setdefault(_value, _name)
...
# Place Embedded assets (generated by the raster core's place_asset).
PLACE_ASSETS: tuple[str, ...] = (
    "Sun flare", "Gradient map", "Noise texture", "Vignette",
)
